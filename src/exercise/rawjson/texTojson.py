...
def infer_problem_type(problem_text: str) -> str:
    ...
    if any(re.search(p, s) for p in value_patterns):
        return "求值题"
    ...
    if any(re.search(p, s) for p in proof_patterns):
        return "证明题"
...
def build_records_from_tex(
    ...
    *,
    ...
    for row in ordered:
        ...
        if include_raw:
            item["raw_problem_tex"] = problem
            item["raw_proof_tex"] = (row.get("raw_proof_tex") or "").strip()
        out.append(item)
        idx += 1
...
def apply_llm_direct_answer_fallback(
    ...
    *,
    ...
    - only for 求值题
    - only when direct_answer is empty
    """
    if not enable:
        return rows
    ...
    for row in rows:
        ptype = row.get("题目类型")
        da = str(row.get("direct_answer") or "").strip()
        if (
            touched < max_items
...
def run_iterative_self_repair(
    ...
    *,
    ...
    llm_direct_answer_max_items: int = 20,
) -> Tuple[List[Dict], List[Dict]]:
    """
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Use LLM as fallback extractor when direct_answer is empty for 求值题",
    )
    ap.add_argument(
        "--llm-direct-answer-max-items",
        type=int,
        default=20,
        help="Max rows for LLM direct_answer fallback",
    )
    ap.add_argument(
        "--require-clean",
        action="store_true",
