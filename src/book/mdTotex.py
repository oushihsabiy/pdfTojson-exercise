...
_EXERCISE_NUM_RE = re.compile(r"^\s*Exercise\s+(?P<num>\d+(?:\.\d+)*)\s*$", re.IGNORECASE)
...
def _build_block_open(type_name: str, label: str, *, exercise: str = "", part: str = "") -> str:
...
def _wrap_subpart_block(type_name: str, exercise_num: str, part_id: str, latex_body: str) -> str:
    ...
    open_line = _build_block_open(type_name, label, exercise=exercise_num, part=part_id)
...
def warn_subpart_mismatch(tex: str) -> None:
...
def main() -> None:
        ...
        if blk.kind == "proof":
            ...
            if p_md:
                ...
                if enable_llm_resegment and expected_parts and (set(expected_parts) - set(found_parts)):
                    ...
                    p_prelude_md, p_parts_md = split_subparts(p_md_checked)
    ...
    warn_subpart_mismatch(body_joined)
