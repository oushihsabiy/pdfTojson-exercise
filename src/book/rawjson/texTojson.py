...
SAFE_EQREF_RE = re.compile(
    r"(?<!\\)\b(?:Eq\.|Eqs\.|Equation|Equations|problem|constraint|update|formula|model)\s*\(\s*(\d+(?:\.\d+)*)\s*\)",
    re.IGNORECASE,
)
BARE_TAG_RE = re.compile(r"(?<!\\)\((\d+(?:\.\d+)*)\)")
...
def build_doc_tag_index(tex: str) -> Dict[str, Dict[str, Any]]:
    ...
    for m in TAG_RE.finditer(tex or ""):
        ...
        out[tag] = {
            ...
            "equation_content": _extract_tagged_equation_block(tex, tag),
            "position": m.start(),
        }
    return out
...
def extract_body_refs(raw_tex: str, known_tags: Dict[str, Dict[str, Any]]) -> Tuple[List[str], Dict[str, str]]:
    txt = raw_tex or ""
    refs: List[str] = []
    evidence: Dict[str, str] = {}
    seen: Set[str] = set()

    for m in SAFE_EQREF_RE.finditer(txt):
        tag = _normalize_tag_token(m.group(1) or "")
        if tag in known_tags and tag not in seen:
            seen.add(tag)
            refs.append(tag)
            evidence[tag] = (m.group(0) or "").strip()

    for m in BARE_TAG_RE.finditer(txt):
        tag = _normalize_tag_token(m.group(1) or "")
        if tag in known_tags and tag not in seen:
            seen.add(tag)
            refs.append(tag)
...
def compose_problem_with_context(problem: str, targets: List[Dict[str, Any]]) -> str:
    p = (problem or "").strip()
    if not targets:
        return p
    ...
    if len(targets) == 1:
        ...
        head = f"Reference formula {t.get('display_tag', '')}: {eq_text}."
    else:
        refs: List[str] = []
        for t in targets:
            eq_text = (t.get("equation_content") or t.get("content") or "").strip()
            refs.append(f"{t.get('display_tag', '')} {eq_text}".strip())
        head = "Reference formulas: " + "; ".join(refs) + "."

    return (head + " Problem: " + p).strip()
...
def build_records_from_tex(
    ...
    *,
    ...
    for row in ordered:
        ...
        body_refs, body_ref_evidence = extract_body_refs(raw_joined, doc_tag_index)
        body_ref_targets = [dict(doc_tag_index[t]) for t in body_refs if t in doc_tag_index]
        problem_with_context = compose_problem_with_context(problem, body_ref_targets)
        ...
        item: Dict[str, Union[int, str, List, Dict]] = {
            ...
            "body_refs": body_refs,
            "body_ref_labels": [doc_tag_index[t]["label_key"] for t in body_refs if t in doc_tag_index],
