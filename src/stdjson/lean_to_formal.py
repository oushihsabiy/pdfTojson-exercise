...
"""Rewrite the 'problem' field of exercise JSON objects from Lean-oriented
structured style into natural-language formal optimization statements.

Usage (standalone):
    python lean_to_formal.py input.json output.json [--prompt PATH] [--model NAME]

When invoked from this project pipeline, the positional arguments can be
provided by the caller similarly to other stdjson scripts.
"""
...
You are given a JSON object representing an optimization exercise.

Task:
- Rewrite only the value of the "problem" field.
- Convert Lean-style segmented wording (e.g., Definition/Hypothesis/Goal)
  into concise, coherent textbook-style mathematical English.
- Preserve all mathematical assumptions and constraints exactly.
- Keep every non-"problem" field unchanged.

Rules:
- Do not solve the problem.
- Do not add explanations, hints, or extra fields.
- Keep LaTeX-compatible notation.
...
def find_prompt_path(prompt_arg: Optional[str]) -> Path:
    ...
    ).parents[1] / "prompt" / "lean_to_formal.md"
...
def chat_completion_text(
    ...
    # Prefer JSON mode when backend supports it.
    try:
...
def build_single_object_prompt(base_prompt: str, exercise: Dict[str, Any], feedback: str = "") -> str:
    ...
    return (
        ...
        " Rewrite the \"problem\" field into concise natural-language formal mathematical style"
        " used in optimization exercises."
        f"{extra}"
        "\n\nInput JSON object:\n"
        f"{object_json}"
    )
...
def formalize_problem_field(
    ...
    raise RuntimeError(
        f"Failed to obtain a valid formalized problem after {max_attempts} attempts: {last_error}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rewrite only the 'problem' field from Lean-structured style to standard natural-language formal statements.",
    )
    ...
    parser.add_argument(
        "--prompt",
        default=None,
        help="Path to the prompt markdown file. Defaults to src/prompt/lean_to_formal.md.",
    )
    parser.add_argument(
        "--model",
        default=None,
...
def main() -> None:
    ...
    for index, exercise in enumerate(exercises, start=1):
        ...
        print(f"[{index}/{total}] formalizing problem for {label}",
              file=sys.stderr)
        try:
            new_problem = formalize_problem_field(
                client,
                model=model,
            ...
            print(
                f"[warn] formalizing failed for {label}; keep original problem. reason={err}",
                file=sys.stderr,
            )

    write_json(output_path, data)
    print(f"Wrote {total} formalized exercise(s) to {output_path}",
          file=sys.stderr)
    if failed_labels:
        print(
