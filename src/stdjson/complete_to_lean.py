...
"""Rewrite the 'problem' field of exercise JSON objects into a structured
format that is easier to translate into Lean.  Uses the complete_to_lean prompt.

Usage (standalone):
    python complete_to_lean.py input.json output.json [--prompt PATH] [--model NAME]
...
You are given a JSON object representing an optimization problem.

Your task is to rewrite only the "problem" field so that it becomes easier to
translate into Lean.  All other fields must be copied exactly without change.

Rewriting rules:
1. Remove unnecessary narrative or pedagogical wording.
2. Split the statement into small information units using labels:
   - Definition: object declarations, domains, function definitions.
   - Hypothesis: constraints, assumptions, inequalities.
   - Goal: "show", "prove", "determine", etc.
3. Keep the mathematical content complete and concise.
4. Preserve all original mathematical symbols and notation.
5. The value of "problem" must remain a single string.
...
def find_prompt_path(prompt_arg: Optional[str]) -> Path:
    ...
    # Default: src/prompt/complete_to_lean.md (one level above stdjson/)
    default_path = Path(__file__).resolve(
    ).parents[1] / "prompt" / "complete_to_lean.md"
...
def lean_rewrite_problem(
    ...
    raise RuntimeError(
        f"Failed to obtain a valid lean-rewritten problem after {max_attempts} attempts: {last_error}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rewrite the 'problem' field into structured Definition/Hypothesis/Goal format for Lean translation.",
    )
    ...
    parser.add_argument(
        "--prompt",
        default=None,
        help="Path to the prompt markdown file. Defaults to src/prompt/complete_to_lean.md.",
    )
    parser.add_argument(
        "--model",
        default=None,
...
def main() -> None:
    ...
    for index, exercise in enumerate(exercises, start=1):
        ...
        print(f"[{index}/{total}] lean-rewriting problem for {label}",
              file=sys.stderr)
        try:
            ...
            print(
                f"[warn] lean-rewriting failed for {label}; keep original problem. reason={err}",
                file=sys.stderr,
            )

    write_json(output_path, data)
    print(f"Wrote {total} lean-rewritten exercise(s) to {output_path}",
          file=sys.stderr)
    if failed_labels:
        print(
